...
estimate_eta() {
  ...
  # This is read-only speed (used for SMART test estimation)
  local pass_s
  ...
  # SMART test duration estimates (based on actual 28TB drive data)
  # SMART short: ~1 minute, SMART long: ~37 hours (full disk scan)
  local smart_short_s=60  # ~1 minute
  local smart_long_s="$pass_s"  # Long test scans entire disk, similar to read-only pass
  local smart_conveyance_s=300  # ~5 minutes (if supported, often not via USB)
  
  # Calculate component times
  local pre_smart_s=0
  local bb_total_s=0
  local post_smart_s=0
  local total_s=0
  
  # Pre-badblocks SMART tests
  pre_smart_s=$((smart_short_s + smart_long_s))
  if [[ "${SMART_CONVEYANCE:-0}" == "1" ]]; then
    pre_smart_s=$((pre_smart_s + smart_conveyance_s))
  fi
  
  # badblocks (if enabled and rotational)
  local bb_pass_s=0
  local write_overhead_mult=130  # 1.30x (30% overhead) - based on actual 28TB drive data
  if [[ "$passes" -gt 0 ]]; then
    # badblocks does write+read, which is slower than read-only.
    # Based on actual 28TB drive data: read-only ~28h, badblocks pass ~37h (1.32x slower).
    # Apply a conservative multiplier to account for write overhead, USB bridge latency, etc.
    bb_pass_s=$(( (pass_s * write_overhead_mult) / 100 ))
    bb_total_s=$((bb_pass_s * passes))
  fi
  
  # Post-badblocks SMART tests (same as pre-badblocks)
  if [[ "$passes" -gt 0 ]]; then
    post_smart_s=$((smart_short_s + smart_long_s))
    if [[ "${SMART_CONVEYANCE:-0}" == "1" ]]; then
      post_smart_s=$((post_smart_s + smart_conveyance_s))
    fi
  fi
  
  # Total time
  total_s=$((pre_smart_s + bb_total_s + post_smart_s))
  
  # Output breakdown
  echo "Estimated full-disk pass time (read-only): ~${pass_s}s (~$((pass_s/3600))h)"
  echo ""
  echo "Estimated component times:"
  echo "  Pre-badblocks SMART tests: ~${pre_smart_s}s (~$((pre_smart_s/3600))h)"
  echo "    - Short test: ~${smart_short_s}s"
  if [[ "${SMART_CONVEYANCE:-0}" == "1" ]]; then
    echo "    - Conveyance test: ~${smart_conveyance_s}s"
  fi
  echo "    - Long test: ~${smart_long_s}s (~$((smart_long_s/3600))h)"
  
  if [[ "$passes" -gt 0 ]]; then
    local bb_pass_h=$((bb_pass_s / 3600))
    echo "  badblocks (${BB_PATTERNS}, ${passes} passes, write+read): ~${bb_total_s}s (~$((bb_total_s/3600))h)"
    echo "    - Per pass: ~${bb_pass_s}s (~${bb_pass_h}h), ~$((write_overhead_mult - 100))% slower than read-only"
    echo "  Post-badblocks SMART tests: ~${post_smart_s}s (~$((post_smart_s/3600))h)"
    echo "    - Short test: ~${smart_short_s}s"
    if [[ "${SMART_CONVEYANCE:-0}" == "1" ]]; then
      echo "    - Conveyance test: ~${smart_conveyance_s}s"
    fi
    ...
    echo "  badblocks: disabled or non-rotational"
  fi
  
  echo ""
  echo "Estimated TOTAL time: ~${total_s}s (~$((total_s/3600))h / ~$((total_s/86400)) days)"
}
...
run_badblocks() {
  # Validate and remap device if needed (USB renumbering can break symlinks)
  # First, try to resolve any symlinks to get the actual device node
  local resolved_dev
  resolved_dev="$(readlink -f -- "$DEVICE" 2>/dev/null || echo "$DEVICE")"
  
  # If resolved path doesn't exist or isn't a block device, try to remap by serial
  if [[ ! -b "$resolved_dev" ]]; then
    log_note "Device path invalid or broken symlink: $DEVICE -> $resolved_dev, attempting remap by serial..."
    if ! ensure_device_present; then
      write_status "badblocks_failed" "badblocks could not start (device missing: $DEVICE)" 0
      die "Device missing: $DEVICE (serial $SERIAL). If it was renumbered, re-run with a stable path like /dev/disk/by-id/..."
    fi
    # ensure_device_present may have remapped DEVICE, resolve it again
    resolved_dev="$(readlink -f -- "$DEVICE" 2>/dev/null || echo "$DEVICE")"
  else
    # Device exists, but still validate it's the right one by serial
    if ! ensure_device_present; then
      write_status "badblocks_failed" "badblocks could not start (device validation failed: $DEVICE)" 0
      die "Device validation failed: $DEVICE (serial $SERIAL)"
    fi
    # Re-resolve in case ensure_device_present remapped it
    resolved_dev="$(readlink -f -- "$DEVICE" 2>/dev/null || echo "$DEVICE")"
  fi
  
  if [[ ! -b "$resolved_dev" ]]; then
    write_status "badblocks_failed" "badblocks could not start (device path invalid: $DEVICE -> $resolved_dev)" 0
    die "Device path invalid: $DEVICE -> $resolved_dev (serial $SERIAL)"
  fi
  ...
  if [[ "$resolved_dev" != "$DEVICE" ]]; then
    log_note "Using resolved device path: $DEVICE -> $resolved_dev"
  fi
  ...
  # Use stdbuf to disable buffering so carriage-return progress updates work correctly
  # (-o0 = unbuffered output, needed because badblocks uses \r to overwrite lines, not \n)
  if stdbuf -o0 -e0 "${bb_cmd[@]}" 2>&1 | tee -a "$LOG_FILE"; then
